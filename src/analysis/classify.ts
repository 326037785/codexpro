...
export function classifyFileRole(filePath: string, language = classifyLanguage(filePath)): AnalysisFileRole {
  ...
  if (CONFIG_NAMES.has(basename) || /\.(?:sln|vcxproj|csproj)$/i.test(basename) || /(^|\/)(config|configs)\//i.test(filePath)) return "config";
...
export function detectProjectTypes(files: InventoryFile[]): string[] {
  ...
  const hasSolution = files.some((file) => file.path.toLowerCase().endsWith(".sln"));
  const hasCsproj = files.some((file) => file.path.toLowerCase().endsWith(".csproj"));
  const hasVcxproj = files.some((file) => file.path.toLowerCase().endsWith(".vcxproj"));
  const hasCSharp = files.some((file) => file.language === "csharp");
  const hasCpp = files.some((file) => file.language === "c" || file.language === "cpp");
  ...
  if (hasCsproj || (hasSolution && hasCSharp && !hasVcxproj)) detected.push("dotnet");
  if (hasVcxproj || (hasSolution && hasCpp)) detected.push("msvc-cpp");
